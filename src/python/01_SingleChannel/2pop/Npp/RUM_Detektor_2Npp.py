...
currpath = '/usr/wrk/people9/tiku2449/EI_RUM/001_Unifying_Framework/RUM_Exploration/2pop/2pop_Npp'
os.chdir(currpath)
...
length_range = np.arange(0,1501,10)
intensity_range = np.arange(0,251,2)
...
for ins in intensity_range:
    ...
    for le in length_range:
        ...
        state_grid[i,j,2] = np.min(signal[1000:3500,0]/560)
        state_grid[i,j,3] = np.max(signal[1000:3500,0]/560)      
...
np.save('RUM_Dec_meas_Npp113k4_le0t1500i10msInt0t250i2_He%.2fmV_Hi%.1fmV.npy' %(he*1000,hi*1000),state_grid)
